...
void AcceptanceandRecoEff_match_FONLLweight_BfeeddownRAAscale()
{
    ...
    if( isPrompt )
		...
		input_fonllweight = new TFile("D0_PbPb_rawtoFONLL_3to100_Bfeeddown_RAAscale.root");
...
////	   cfg_d0efficiency_ypt->SaveAs("plots/acceptandeff/D0_PbPb_efficiency_ypt.png");
   ...
   char outputfile[200];
   if( isPrompt )
	   sprintf(outputfile,"rootfiles/D0_PbPb_acc_eff_ptbin_%d_ybin_%d_prompt_FONLLweight_cent%2.0fto%2.0f.root", NPT, NY, hiBin_low * 0.5, hiBin_high * 0.5);
   else
	   sprintf(outputfile,"rootfiles/D0_PbPb_acc_eff_ptbin_%d_ybin_%d_Bfeeddown_FONLLweight_cent%2.0fto%2.0f_RAAscale.root", NPT, NY, hiBin_low * 0.5, hiBin_high * 0.5);
