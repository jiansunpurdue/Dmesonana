void Dmeson_RAFONLL()
{
	...
	gStyle->SetOptStat(0);
	TH1::SetDefaultSumw2();
	TFile * input_pbpb = new TFile("Spectrum_Recoeff_D0_PbPb_dpt_evtunprescaleMB0.root");
	...
	D0_pbpb_spectrum->Scale(1.0/1.127E9);   //number of MB events in PbPb

	D0_pbpb_spectrum->Scale(1.0E9/5.67);
...
//	}
	...
	for( int i = 0; i < D0_fonll_raa_errordata->GetN();i++){
        ...
        cout << "PbPb: " << D0_pbpb_spectrum_raa->GetBinContent(i+2) << " pp: " << d0fonllspectrum->GetY()[i] << " ratio: " << D0_fonll_raa_errorfonll->GetY()[i] << endl;
	...
	Tl.DrawLatex(0.3,0.7, "Centrality 0-100%");
