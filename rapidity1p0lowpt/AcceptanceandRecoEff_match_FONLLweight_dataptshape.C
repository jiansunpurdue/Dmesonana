...
//just work for prompt D0

bool isPrompt = true;

float hiBin_low = -0.5;
float hiBin_high = 199.5;
double dautrackptcut = 0.4;

double lowptedge_d0 = 0.0;
...
//
//           double weight_data_fonll = fit_fun_datafitted->Eval(dpt[igend])/fonllspectrum->GetBinContent( fonllspectrum->FindBin( dpt[igend] ));
//		   weight = weight * weight_data_fonll;
		   ...
		   if( pt_ddau[igend][0] > dautrackptcut && pt_ddau[igend][1] > dautrackptcut )
...
//	   cfg_d0accxeff_ypt->SaveAs("plots/acceptandeff/D0_PbPb_accxeff_ypt.png");
   ...
   char outputfile[200];
   if( isPrompt )
	   sprintf(outputfile,"rootfiles/D0_PbPb_acc_eff_ptbin_%d_ybin_%d_prompt_FONLLweight_cent%2.0fto%2.0f_dataptshape.root", NPT, NY, hiBin_low * 0.5, hiBin_high * 0.5);
   else
	   sprintf(outputfile,"rootfiles/D0_PbPb_acc_eff_ptbin_%d_ybin_%d_Bfeeddown_FONLLweight_cent%2.0fto%2.0f_dataptshape.root", NPT, NY, hiBin_low * 0.5, hiBin_high * 0.5);
