...
double eff_fit_range_low[NPT] = {1.70, 1.80, 1.75, 1.72, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high[NPT] = {2.05, 1.95, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05};
...
void fit_hist( TH1F * histo, TCanvas *cfg, int iptbin , TH1D * counts, float lowrange, float highrange)
{
    ...
    TF1* fit_fun = new TF1("fit_fun", "gausn(0) + gausn(3) * ([0]/0.01) * (1.0/[6]) + expo(7)", fit_range_low, fit_range_high);
...
//        fit_fun_doublecounted->Draw("same");
        ...
        TF1* fit_fun_doubleplusbkg = new TF1("fit_fun_doubleplusbkg", "gausn(0) * ([3]/0.01) * (1.0/[4]) + expo(5)", fit_range_low, fit_range_high);
...
void FillSpectrum_doublecountedexpo()
{
	book_hist();
...
//   gStyle->SetHistMinimumZero(kTRUE);
   ...
   char cfgname[200];
   sprintf(cfgname,"plots/D0_PbPb_data_ptbin_%d_ptd_unpreMBtrig_%d_cent%2.0fto%2.0f_doublecountedexpo.pdf",NPT, evtunprescaleMB, hiBin_low * 0.5, hiBin_high * 0.5);
   cfg_mb->SaveAs(cfgname);
   sprintf(cfgname,"plots/D0_PbPb_data_ptbin_%d_ptd_unpreMBtrig_%d_cent%2.0fto%2.0f_doublecountedexpo.gif",NPT, evtunprescaleMB, hiBin_low * 0.5, hiBin_high * 0.5);
   cfg_mb->SaveAs(cfgname);
  
   char outputfile[200];
   sprintf(outputfile,"rootfiles/Dspectrum_pbpb_data_ptbin_%d_ptd_unpreMBtrig_%d_cent%2.0fto%2.0f_doublecountedexpo.root", NPT, evtunprescaleMB, hiBin_low * 0.5, hiBin_high * 0.5);
