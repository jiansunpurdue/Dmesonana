...
int weight_cal_pythiarawtofonll()
{
  ...
  TFile * input_PbPb_MC = new TFile("Dmesonana_hiforest_PbPb_Pyquen_D0embedded_D0pt3_pthat015305080_1217_1223_all_v1.root");
  ...
  gendmesontree->Draw("dpt>>D0_pythiaspectrum"," weight_pthat * (dpt<100 && abs(dy)<2)","goff");

  TH1D * D0_pythiaspectrum_raw = new TH1D("D0_pythiaspectrum_raw","D0_pythiaspectrum_raw",BIN_NUM,HMIN,HMAX);
  gendmesontree->Draw("dpt>>D0_pythiaspectrum_raw","dpt<100 && abs(dy)<2","goff");
  ...
  gPad->SetLogy();
  D0_pythiaspectrum->GetXaxis()->SetRangeUser(3.0, 100);
