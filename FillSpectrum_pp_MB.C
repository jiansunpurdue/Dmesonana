...
void FillSpectrum_pp_MB()
{
	book_hist();

    TFile * input = new TFile("Dmesonana_listppMB_merge.lis_0To1.root");
   ...
   for ( int entry = 0; entry < recodmesontree->GetEntries(); entry++ )
   {
	   ...
	   if( !MinBias)  {  cout << "error!!!!!!!!!" << endl;  continue;}
   ...
   char outputfile[200];
   sprintf(outputfile,"Dspectrum_ppMB_histo_ptbin_%d.root", NPT);
