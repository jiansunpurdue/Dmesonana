...
//  hs_allD_mc->GetXaxis()->SetLimits(min, max);
  ...
  c->SaveAs(Form("plots_pt3p5to7/varcomparison_bothsideband_%s_pt3p5to7_allcut_datapt_nodouble.pdf",varname.Data()));
  c->SaveAs(Form("plots_pt3p5to7/varcomparison_bothsideband_%s_pt3p5to7_allcut_datapt_nodouble.png",varname.Data()));
}
...
void DrawComparison_bothsideband_promptBtoD()
{
  ...
  gStyle->SetHistMinimumZero(kTRUE);
      ...
      TFile * input_mc_prompt = new TFile("filespt3p5to7/Cut_distribution_MC_FONLLweight_Prompt_pt4to7_sideband_dataptweight_nodouble.root");
	  TFile * input_mc_BtoD = new TFile("filespt3p5to7/Cut_distribution_MC_FONLLweight_BtoD_pt4to7_sideband_dataptweight_nodouble.root");
