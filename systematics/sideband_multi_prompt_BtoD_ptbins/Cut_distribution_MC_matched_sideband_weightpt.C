...
void Cut_distribution_MC_matched_sideband_weightpt()
{
	TH1::SetDefaultSumw2();
    ...
    TFile * input_ptweight = new TFile("/home/sun229/DmesonAna/sl6ana/v1_pbpb_pp_pPb/CMSSW_5_3_20/src/UserCode/OpenHF/Dmesonana/recoeff_pthatweight_fonllweight/RAA_0331/RAA_D0_PbPb_spectrum_fonll_effunpre_cent0to100_ptbin9.root");
    TGraphAsymmErrors * D0_fonll_raa_errordata = ( TGraphAsymmErrors * ) input_ptweight->Get("D0_fonll_raa_errordata");
    TF1 * fit_fun = D0_fonll_raa_errordata->GetFunction("fit_fun");
    cout << "p0:  " << fit_fun->GetParameter(0) << "   p1: " << fit_fun->GetParameter(1) << "   p2: " << fit_fun->GetParameter(2) << endl;
...
//		   if( matchedtogen->at(icand) == 1 && nongendoublecounted->at(icand) == 0 )  continue;
           ...
           double ptweight = fit_fun->Eval(dcandpt->at(icand));
           weight = weight * ptweight;
//         cout << "Pt: " << dcandpt->at(icand) << "ptweight : " << ptweight << endl;
...
//               continue;
   ...
   char outputfile[200];
   if( isPrompt )
       sprintf(outputfile,"Cut_distribution_MC_FONLLweight_Prompt_pt%1.0fto%1.0f_sideband_weightpt.root", Ptcut_low, Ptcut_high);
   else
       sprintf(outputfile,"Cut_distribution_MC_FONLLweight_BtoD_pt%1.0fto%1.0f_sideband_weightpt.root", Ptcut_low, Ptcut_high);
