double sideband_close = 0.10;//0.06;//0.10;//0.10;//0.07; //0.06
double sideband_far = 0.15;//0.10;//0.15;//0.15;//0.10;  //0.11
...
//  c_mass->SaveAs(Form("plotscent-0to30_loosealphacut/varmass_bothsideband_%s_pt7_%s.pdf",varname.Data(),type.Data()));
//  c_mass->SaveAs(Form("plotscent-0to30_loosealphacut/varmass_bothsideband_%s_pt7_%s.png",varname.Data(),type.Data()));
...
//  hs_allD_mc->GetXaxis()->SetLimits(min, max);
  ...
  c->SaveAs(Form("plotsclosersideband/varcomparison_cent-0to30_bothsideband_%s_pt7to40_allcut_datapt_nodouble.pdf",varname.Data()));
  c->SaveAs(Form("plotsclosersideband/varcomparison_cent-0to30_bothsideband_%s_pt7to40_allcut_datapt_nodouble.png",varname.Data()));
}
...
void DrawComparison_bothsideband_promptBtoD()
{
  ...
  TFile * input_data = new TFile("filesclosersideband/Cut_distribution_Data_sideband_pt7to40_rapidity0to1_cent-0to30.root");
  TFile * input_mc_prompt = new TFile("filesclosersideband/Cut_distribution_MC_FONLLweight_Prompt_pt7to40_rapidity0to1_cent-0to30_sideband_dataptweight.root");
  TFile * input_mc_BtoD = new TFile("filesclosersideband/Cut_distribution_MC_FONLLweight_BtoD_pt7to40_rapidity0to1_cent-0to30_sideband_dataptweight.root");

//   Drawvariable("y","rapidity",input_data,input_mc_prompt, input_mc_BtoD, -1, 1, 2, -1, 1, -1, 1);
//    Drawvariable("ffls3d","decay length sig",input_data,input_mc_prompt, input_mc_BtoD, 0.0, 40, 20, 0, 1000, 5.0, 1000);
