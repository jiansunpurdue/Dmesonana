...
void fit_hist( TH1F * histo, TCanvas *cfg, float ptlow, float pthigh)
{
  ...
  TF1* fit_fun = new TF1("fit_fun", "[0]*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[5]*x*x", cut_m_low, cut_m_high);
  float max = histo->GetMaximum();
  
  float p0=1000, p1=1.87, p2=0.02, p8=0.05;
  float p7=0.5;
  float p0_L=0, p1_L=1.84, p2_L=0, p8_L=0;
  float p7_L=0;
  float p0_H = 2*max, p1_H = 1.9, p2_H = 0.05, p8_H=0.08;
  float p7_H=1;
  ...
  while (!pass)
      ...
      if(!pass) 
	{
	  ...
	  p8 = fit_fun->GetParameter(8);
	  p7 = fit_fun->GetParameter(7);
        }
...
void fit_DoubleGau()
{
  ...
  cfg_mb->SaveAs(Form("plots/doubleGau/massSpectrum_%.0fp0_%.0fp0.pdf",channel.Data(),func.Data(),ptmin,ptmax));
