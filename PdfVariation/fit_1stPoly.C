...
void fit_hist( TH1F * histo, TCanvas *cfg, float ptlow, float pthigh)
{
  ...
  TF1* fit_fun = new TF1("fit_fun", "gausn(0) + pol1(3)", cut_m_low, cut_m_high);
...
void fit_1stPoly()
{
  ...
  cfg_mb->SaveAs(Form("plots/1stPoly/massSpectrum_%.0fp0_%.0fp0.pdf",ptmin,ptmax));
