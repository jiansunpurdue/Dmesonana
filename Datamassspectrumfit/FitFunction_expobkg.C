...
double eff_fit_range_low_expobkg_ptbin12[12] = {1.70, 1.77, 1.75, 1.72, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70};
//double eff_fit_range_high_expobkg_ptbin12[12] = {2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05};
double eff_fit_range_high_expobkg_ptbin12[12] = {2.05, 1.95, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05};

double eff_fit_range_low_expobkg_ptbin7[7] = {1.70, 1.77, 1.75, 1.70, 1.70, 1.70, 1.70};
//double eff_fit_range_high_expobkg_ptbin7[7] = {2.05, 2.05, 2.00, 2.05, 2.05, 2.05, 2.05};
double eff_fit_range_high_expobkg_ptbin7[7] = {2.05, 1.95, 2.00, 2.05, 2.05, 2.05, 2.05};
...
double eff_fit_range_low_expobkg_ptbin9[9] = {1.70, 1.70, 1.77, 1.75, 1.70, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high_expobkg_ptbin9[9] = {2.05, 2.05, 2.00, 2.00, 2.05, 2.05, 2.05, 2.05, 2.05};

void fit_hist_expobkg( TH1F * histo, TCanvas *cfg, int iptbin , TH1D * rawdNdpt, TH1D * rawdNdpt_counting, TH1F * hfg_massmean, TH1F * hfg_masssigma, int cent_low, int cent_high)
{
    double *eff_fit_range_low_expobkg = new double[NPT];
    double *eff_fit_range_high_expobkg = new double[NPT];

	for( int ibin = 0; ibin < NPT; ibin++ )
	{
		eff_fit_range_low_expobkg[ibin] = 1.70;
		eff_fit_range_high_expobkg[ibin] = 2.05;
	}
	...
	if( NPT == 12 )
	{
		for( int ibin = 0; ibin < NPT; ibin++ )
		{
			eff_fit_range_low_expobkg[ibin] = eff_fit_range_low_expobkg_ptbin12[ibin];
			eff_fit_range_high_expobkg[ibin] = eff_fit_range_high_expobkg_ptbin12[ibin];
		}
	}
	else if ( NPT == 7 )
	{
		for( int ibin = 0; ibin < NPT; ibin++ )
		{
			eff_fit_range_low_expobkg[ibin] = eff_fit_range_low_expobkg_ptbin7[ibin];
			eff_fit_range_high_expobkg[ibin] = eff_fit_range_high_expobkg_ptbin7[ibin];
		}
	}
	else if ( NPT == 14 )
	{
		for( int ibin = 0; ibin < NPT; ibin++ )
		{
			eff_fit_range_low_expobkg[ibin] = eff_fit_range_low_expobkg_ptbin14[ibin];
			eff_fit_range_high_expobkg[ibin] = eff_fit_range_high_expobkg_ptbin14[ibin];
		}
	}
	else if ( NPT == 9 )
	{
		for( int ibin = 0; ibin < NPT; ibin++ )
		{
			eff_fit_range_low_expobkg[ibin] = eff_fit_range_low_expobkg_ptbin9[ibin];
			eff_fit_range_high_expobkg[ibin] = eff_fit_range_high_expobkg_ptbin9[ibin];
		}
	}
	...
	double fit_range_low = eff_fit_range_low_expobkg[iptbin];
	double fit_range_high = eff_fit_range_high_expobkg[iptbin];
	...
	double integral_begin = 1.865 - 3.0 * fitsigma;
	double integral_end = 1.865 + 3.0 * fitsigma;
	int integral_binnumber_begin =  histo->FindBin(integral_begin);
	int integral_binnumber_end = histo->FindBin(integral_end);
	integral_begin = histo->GetBinLowEdge( integral_binnumber_begin);  //change the integral to bin edges
	integral_end = histo->GetBinLowEdge(integral_binnumber_end+1);
	double Nbkg_integral_bkgfun = fit_fun_bgplusmisid->Integral( integral_begin, integral_end ) /  binwidth;
//	double error_Nbkg_integral_bkgfun = fit_fun_bgplusmisid->IntegralError( integral_begin, integral_end ) / binwidth;
	//error_Nbkg_integral_bkgfun is 0 now, not sure why
	double error_Nbkg_integral_bkgfun = TMath::Sqrt(Nbkg_integral_bkgfun);

    double Nentries_counts;
	double error_Nentries_counts;
	Nentries_counts = histo->IntegralAndError( integral_binnumber_begin, integral_binnumber_end, error_Nentries_counts );
	Nentries_counts = Nentries_counts;
	error_Nentries_counts = error_Nentries_counts;

	cout << "  Nentries_counts: " << Nentries_counts << "   error_Nentries_counts: " << error_Nentries_counts << endl;
	cout << "  Nbkg_integral_bkgfun: " << Nbkg_integral_bkgfun << "   error_Nbkg_integral_bkgfun: " << error_Nbkg_integral_bkgfun << endl;

	double dNdpt_counts = ( Nentries_counts - Nbkg_integral_bkgfun ) / ptbinwidth;
	double error_dNdpt_counts = TMath::Sqrt( error_Nbkg_integral_bkgfun * error_Nbkg_integral_bkgfun + error_Nentries_counts * error_Nentries_counts) / ptbinwidth;
//    double error_dNdpt_counts = error_Nentries_counts;

	rawdNdpt_counting->SetBinContent( iptbin+1, dNdpt_counts);
	rawdNdpt_counting->SetBinError( iptbin+1, error_dNdpt_counts);
	

	cout << " integral_begin: " << integral_begin << "   integral_end: " << integral_end << "   integral_binnumber_begin: " << integral_binnumber_begin << "  integral_binnumber_end: " << integral_binnumber_end << endl;

	cout << "N sig fit: " << Nsig << "  error N sig fit: " << err_Nsig << endl; 
	cout << " N sig counts: " << dNdpt_counts * ptbinwidth << " error N sig counts: " << error_dNdpt_counts * ptbinwidth<< endl;
	...
	sprintf( sigma_print, "#sigma: %.5f", fitsigma);
