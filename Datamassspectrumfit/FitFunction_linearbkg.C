...
double eff_fit_range_low_linearbkg_ptbin12[12] = {1.70, 1.78, 1.78, 1.72, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high_linearbkg_ptbin12[12] = {2.05, 1.95, 1.95, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05};

double eff_fit_range_low_linearbkg_ptbin7[7] = {1.70, 1.78, 1.75, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high_linearbkg_ptbin7[7] = {2.05, 1.95, 2.00, 2.05, 2.05, 2.05, 2.05};

double eff_fit_range_low_linearbkg_ptbin14[14] = {1.70, 1.70, 1.77, 1.75, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high_linearbkg_ptbin14[14] = {2.05, 2.05, 2.00, 2.00, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05, 2.05};

double eff_fit_range_low_linearbkg_ptbin9[9] = {1.70, 1.70, 1.77, 1.75, 1.70, 1.70, 1.70, 1.70, 1.70};
double eff_fit_range_high_linearbkg_ptbin9[9] = {2.05, 2.05, 2.00, 2.00, 2.05, 2.05, 2.05, 2.05, 2.05};


void fit_hist_linearbkg( TH1F * histo, TCanvas *cfg, int iptbin , TH1D * rawdNdpt, TH1F * hfg_massmean, TH1F * hfg_masssigma, int cent_low, int cent_high)
{
    double *eff_fit_range_low_linearbkg = new double[NPT];
    double *eff_fit_range_high_linearbkg = new double[NPT];

    for( int ibin = 0; ibin < NPT; ibin++ )
    {
        eff_fit_range_low_linearbkg[ibin] = 1.70;
        eff_fit_range_high_linearbkg[ibin] = 2.05;
    }
    ...
    if( NPT == 12 )
    {
        for( int ibin = 0; ibin < NPT; ibin++ )
        {
            eff_fit_range_low_linearbkg[ibin] = eff_fit_range_low_linearbkg_ptbin12[ibin];
            eff_fit_range_high_linearbkg[ibin] = eff_fit_range_high_linearbkg_ptbin12[ibin];
        }
    }
    else if ( NPT == 7 )
    {
        for( int ibin = 0; ibin < NPT; ibin++ )
        {
            eff_fit_range_low_linearbkg[ibin] = eff_fit_range_low_linearbkg_ptbin7[ibin];
            eff_fit_range_high_linearbkg[ibin] = eff_fit_range_high_linearbkg_ptbin7[ibin];
        }
    }
    else if ( NPT == 14 )
    {
        for( int ibin = 0; ibin < NPT; ibin++ )
        {
            eff_fit_range_low_linearbkg[ibin] = eff_fit_range_low_linearbkg_ptbin14[ibin];
            eff_fit_range_high_linearbkg[ibin] = eff_fit_range_high_linearbkg_ptbin14[ibin];
        }
    }
    else if ( NPT == 9 )
    {
        for( int ibin = 0; ibin < NPT; ibin++ )
        {
            eff_fit_range_low_linearbkg[ibin] = eff_fit_range_low_linearbkg_ptbin9[ibin];
            eff_fit_range_high_linearbkg[ibin] = eff_fit_range_high_linearbkg_ptbin9[ibin];
        }
    }
	...
	double fit_range_low = eff_fit_range_low_linearbkg[iptbin];
	double fit_range_high = eff_fit_range_high_linearbkg[iptbin];
	...
	TF1* fit_fun = new TF1("fit_fun", "[0]*([3]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[3])*Gaus(x,[1],[4])/(sqrt(2*3.14159)*[4])) + gausn(5) * ([0]/0.01) * (1.0/[8]) + pol1(9)", fit_range_low, fit_range_high);
	...
	//misid D0 plus bkg
	TF1* fit_fun_bgplusmisid = new TF1("fit_fun_bgplusmisid", "gausn(0) * ([6]/0.01) * (1.0/[3]) + pol1(4)", fit_range_low, fit_range_high);
