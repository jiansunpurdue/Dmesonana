...
void book_hist()
{
	...
	for(int i = 0; i<NPT; i++)
	{
		...
		sprintf(pt_range, "p_{T}: %2.1f to %2.1f GeV/c", pt_low, pt_high);
...
//		   cout << " pt: " << dcandpt->at(icand) << "  ipt: " << ipt << endl;
   ...
   char outputfile[200];
   sprintf(outputfile,"rootfiles/Masshisto_data_ptbin_%d_ptd_unpreMBtrig_%d_cent%2.0fto%2.0f.root", NPT, evtunprescaleMB, hiBin_low * 0.5, hiBin_high * 0.5);
