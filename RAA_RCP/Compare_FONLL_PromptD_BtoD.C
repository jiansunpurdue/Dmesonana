...
int Compare_FONLL_PromptD_BtoD()
{
...
//      hpt->SetBinContent(i+1,0.557 * max_all[i]);
  ...
  TFile * input_BFONLL = new TFile("./BtoD_fromyenjie/BtoD-0-100.root");
...
//  c_fonll_pythia->cd(1);
  ...
  t->SetFillColor(0);
  t->AddEntry((TObject*)0,"#left|y#right| < 2.0","");
...
//  ratio_pythia_fonll->Draw("same");
  ...
  t1->SetFillColor(0);
//  t1->AddEntry(ratio_pythia_fonll,"Pythia Prompt D0/FONLL Prompt D0");
