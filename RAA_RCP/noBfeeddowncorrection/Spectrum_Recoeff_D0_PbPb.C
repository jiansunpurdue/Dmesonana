...
// for data
    ...
    d0raw_data_overeff->GetYaxis()->SetTitle("#frac{dN}{dp_{T}}");
	...
	for( int i =0; i < d0raw_data_overeff->GetNbinsX(); i++ )
	{
		cout << "  number of D0: " << d0raw_data_overeff->GetBinContent(i+1) << "   error: " << d0raw_data_overeff->GetBinError(i+1) << endl;
	}

// end data
//
    char outputfile[200];
	sprintf(outputfile,"Spectrum_centRecoeff_D0_PbPb_dpt_cent%dto%d_ptbin%d.root", cent_low, cent_high, Nptbin);
