double syserrorPer_upper_ptbin5_cent0to10_RCP[5] = { 0.343433 ,0.233654 ,0.223844 ,0.147709 ,0.191917};
double syserrorPer_lower_ptbin5_cent0to10_RCP[5] = { 0.343433 ,0.233654 ,0.223844 ,0.147709 ,0.191917};

double syserrorPer_upper_ptbin5_cent10to30_RCP[5] = { 0.231642 ,0.201606 ,0.165245 ,0.154055 ,0.166505};
double syserrorPer_lower_ptbin5_cent10to30_RCP[5] = { 0.231642 ,0.201606 ,0.165245 ,0.154055 ,0.166505};

double syserrorPer_upper_ptbin5_cent30to50_RCP[5] = { 0.164493 ,0.119574 ,0.0895433 ,0.122548 ,0.11015};
double syserrorPer_lower_ptbin5_cent30to50_RCP[5] = { 0.164493 ,0.119574 ,0.0895433 ,0.122548 ,0.11015};
...
void Draw_RCP( TFile * input_pbpb_central, TFile * input_pbpb_perip, int cent_low, int cent_high, int Nptbin, double TAA_central, double TAA_perip)
{
    ...
    if( Nptbin == 5 && cent_low == 0 && cent_high == 10 )
    {
        for( int i = 0; i < Nptbin; i++ )
        {
            syserrorPer_upper[i] = syserrorPer_upper_ptbin5_cent0to10_RCP[i];
            syserrorPer_lower[i] = syserrorPer_lower_ptbin5_cent0to10_RCP[i];
        }
    }
    ...
    if( Nptbin == 5 && cent_low == 10 && cent_high == 30 )
    {
        for( int i = 0; i < Nptbin; i++ )
        {
            syserrorPer_upper[i] = syserrorPer_upper_ptbin5_cent10to30_RCP[i];
            syserrorPer_lower[i] = syserrorPer_lower_ptbin5_cent10to30_RCP[i];
        }
    }
    ...
    if( Nptbin == 5 && cent_low == 30 && cent_high == 50 )
    {
        for( int i = 0; i < Nptbin; i++ )
        {
            syserrorPer_upper[i] = syserrorPer_upper_ptbin5_cent30to50_RCP[i];
            syserrorPer_lower[i] = syserrorPer_lower_ptbin5_cent30to50_RCP[i];
        }
    }
	...
	if( Nptbin == 5 || Nptbin == 9 )   
	{
		...
		D0_pbpb_spectrum_perip->GetYaxis()->SetRangeUser(4.0, 24);
	}
...
//      cout << " RAA: " << D0_fonll_raa_errorsys->GetY()[i] << endl;
    ...
    RCP_errorsys->GetXaxis()->SetRangeUser(3.5,28.);
	RCP_errorsys->GetYaxis()->SetTitleOffset(1.25);
    RCP_errorsys->GetYaxis()->SetTitle("Prompt D^{0} R^{P, 50-100%}_{C}");
	...
	D0_Rcp_errorstats->GetYaxis()->SetTitle("Prompt D^{0} R^{P, 50-100%}_{C}");
    ...
    char cfgname[200];
	sprintf(cfgname,"results/RCPD0_PbPb_effunpre_cent%dto%d_ptbin%d.pdf",cent_low,cent_high, Nptbin);
	cfg_D0_Rcp->SaveAs(cfgname);
	sprintf(cfgname,"results/RCPD0_PbPb_effunpre_cent%dto%d_ptbin%d.png",cent_low,cent_high, Nptbin);
	cfg_D0_Rcp->SaveAs(cfgname);
...
// 5 ptbins, used for RAA vs Npart
   ...
   //centrality 0% to 10% 
	  input_pbpb_central = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent0to10_ptbin5.root");
	  input_pbpb_perip = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent50to100_ptbin5.root");
   ...
   //centrality 10% to 30% 
	  input_pbpb_central = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent10to30_ptbin5.root");
	  input_pbpb_perip = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent50to100_ptbin5.root");
   ...
   //centrality 30% to 50% 
	  input_pbpb_central = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent30to50_ptbin5.root");
	  input_pbpb_perip = new TFile("Prompt_Spectrum_centRecoeff_D0_PbPb_dpt_cent50to100_ptbin5.root");
