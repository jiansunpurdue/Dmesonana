...
#define ERROR_SYS_ACCEFF_PERCEN 0.15
double ERROR_SYS_TKEFF_PERCEN = 9999.0;

double ERROR_SYS_FIT_PERCEN_cent0to10_5ptbin[5] = { 31.1e-2, 16.8e-2, 16.6e-2, 7.4e-2, 13.5e-2};
double ERROR_SYS_FIT_PERCEN_cent10to30_5ptbin[5] = { 5.4e-2, 7.8e-2, 6.9e-2, 5.4e-2, 5.6e-2};
double ERROR_SYS_FIT_PERCEN_cent30to50_5ptbin[5] = { 3.6e-2, 6.5e-2, 4.6e-2, 10.3e-2, 6.3e-2};
double ERROR_SYS_FIT_PERCEN_cent50to100_5ptbin[5] = { 7.1e-2, 8.7e-2, 8.9e-2, 9.9e-2, 13.6e-2};
...
void Calculate_Bfraction( int Nptbin, TFile *input_data_raw, TFile *input_mc_BtoDeff, TFile *input_BtoD, int cent_low, int cent_high, double NEVT, double TAA, double ERROR_TAA_PERCEN)
{
    ...
    //Rebin Edge
    double rebin[REBINp] = {3.5, 4.5, 7.0, 11.0, 16.0, 28.0};
    ...
    //bin middle
    double apt[REBIN] = {4.0, 5.75, 9.0, 13.5, 22.0};//pPb_pt
    //bin half width
    double aptl[REBIN] = {0.5, 1.25, 2.0, 2.5, 6.0};//pPb_pt
    double asigma[REBIN],aminall[REBIN],amaxall[REBIN],aerrorl[REBIN],aerrorh[REBIN];
    //number of every rebined bin
    double bin_num[REBIN] = {4, 10, 16, 20, 48};//pPb_pt
...
//		cout << "histo bin width: " << recoeff_BtoD->GetBinWidth(i+2) << "   pt range: " << ptrange << endl;
		...
		double symmetricerrorPer_squaresum = /*syserrorPer_fit[i] * syserrorPer_fit[i] + */ ERROR_SYS_TKEFF_PERCEN * ERROR_SYS_TKEFF_PERCEN + ERROR_SYS_ACCEFF_PERCEN * ERROR_SYS_ACCEFF_PERCEN + ERROR_BR_PERCEN * ERROR_BR_PERCEN + ERROR_TAA_PERCEN * ERROR_TAA_PERCEN + errorPer_eff * errorPer_eff /*+ errorPer_entries_Dzero_raw * errorPer_entries_Dzero_raw*/;
//		cout << "fit error per: " << syserrorPer_fit[i] << endl;
...
//		cout << " bfraction: " << bfraction[i] << endl;
    ...
    gaeBfraction->GetXaxis()->SetRangeUser(3.5, 28);
    ...
    gaePromptfraction->GetXaxis()->SetRangeUser(3.5, 28);
    ...
    gaeSigmaBtoD->GetXaxis()->SetRangeUser(3.5, 28);
    ...
    sprintf(outputfile,"rootfiles/Bfraction_method1_ptbin_%d_cent%dto%d.root", NPT, cent_low, cent_high);
...
void Bfractioncal_method1_ptbin7()
{
	int cent_low = 0;
	int cent_high = 0;
    double NEVT = 0.0;
	...
	TFile * input_data_raw = new TFile("./../Datamassspectrumfit/rootfiles/Dspectrum_pbpb_data_ptbin_7_ptd_unpreMBtrig_0_cent0to10.root");
	TFile * input_mc_BtoDeff = new TFile("./../rootfiles/D0_PbPb_acc_eff_ptbin_7_ybin_6_Bfeeddown_FONLLweight_cent-0to10_RAAscale.root");
	...
	TFile * input_data_raw = new TFile("./../Datamassspectrumfit/rootfiles/Dspectrum_pbpb_data_ptbin_7_ptd_unpreMBtrig_0_cent10to30.root");
	TFile * input_mc_BtoDeff = new TFile("./../rootfiles/D0_PbPb_acc_eff_ptbin_7_ybin_6_Bfeeddown_FONLLweight_cent10to30_RAAscale.root");
	TFile * input_BtoD = new TFile("./BtoD_fromyenjie/BtoD-10-30.root");
    Calculate_Bfraction( Nptbin, input_data_raw, input_mc_BtoDeff, input_BtoD, cent_low, cent_high, NEVT, TAA, ERROR_TAA_PERCEN);
    ...
    TFile * input_data_raw = new TFile("./../Datamassspectrumfit/rootfiles/Dspectrum_pbpb_data_ptbin_7_ptd_unpreMBtrig_0_cent30to50.root");
    TFile * input_mc_BtoDeff = new TFile("./../rootfiles/D0_PbPb_acc_eff_ptbin_7_ybin_6_Bfeeddown_FONLLweight_cent30to50_RAAscale.root");
    TFile * input_BtoD = new TFile("./BtoD_fromyenjie/BtoD-30-50.root");
    Calculate_Bfraction( Nptbin, input_data_raw, input_mc_BtoDeff, input_BtoD, cent_low, cent_high, NEVT, TAA, ERROR_TAA_PERCEN);
    ...
    TFile * input_data_raw = new TFile("./../Datamassspectrumfit/rootfiles/Dspectrum_pbpb_data_ptbin_7_ptd_unpreMBtrig_0_cent50to100.root");
    TFile * input_mc_BtoDeff = new TFile("./../rootfiles/D0_PbPb_acc_eff_ptbin_7_ybin_6_Bfeeddown_FONLLweight_cent50to100_RAAscale.root");
    TFile * input_BtoD = new TFile("./BtoD_fromyenjie/BtoD-50-100.root");
    Calculate_Bfraction( Nptbin, input_data_raw, input_mc_BtoDeff, input_BtoD, cent_low, cent_high, NEVT, TAA, ERROR_TAA_PERCEN);
